...
The log-variance is clamped to $[-4, 4]$ before the exponential, constraining
the effective variance to $[e^{-4}, e^{4}]$. Wider clamps allowed the model to
claim near-infinite uncertainty, collapsing the uncertainty map to a spatially
uniform constant. The NLL loss is used with $\lambda_{\text{recon}} = 1.0$
(not $100$ as in the L1 baseline), since the $\log \sigma^2$ term
self-normalizes the loss magnitude; using $\lambda_{\text{recon}} = 100$ drove
the loss to $-200$ and caused complete uncertainty collapse.

Training uses a two-phase strategy: the backbone is frozen for 10 epochs while
...
10 further epochs. Gradient clipping ($\|\nabla\|_2 \leq 1.0$) is applied
to stabilize Phase~2. A \texttt{ReduceLROnPlateau} scheduler halves the
learning rate after 3 epochs without PSNR improvement, and early stopping
(patience 5) prevents overfitting.
