...
class MainModel(nn.Module):
    ...
    lambda_TV : float
        Weight on Total Variation loss. Increase to 2-5 to reduce color artifacts.
    ...
    def backward_D(self):
        ...
        fake_preds = self.net_D(fake_image.detach())  # detach so G gets no gradient from D step
    ...
    def backward_G(self):
        ...
        self.loss_G_GAN = self.GANcriterion(fake_preds, True)  # G wants D to say "real"
        self.loss_G_L1 = self.L1criterion(self.fake_color, self.ab) * self.lambda_L1

        # TV loss on stacked [L | fake_ab] to smooth color transitions
    ...
    def optimize(self):
        ...
        # Update generator
        self.net_G.train()
        self.set_requires_grad(self.net_D, False)  # freeze D while updating G
